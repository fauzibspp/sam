...
function barcode_outimage($folder = '', $text, $bars, $scale = 1, $mode = "png",$total_y = 0, $space = ''){
    ...
    if ($mode=='jpg' || $mode=='jpeg'){
        $barcode_serial = '../../documents/'.$folder.'/barcode_serial.jpg';
        header("Content-Type: image/jpeg; name=\"$barcode_serial\"");
        imagejpeg($im,$barcode_serial);

    } else if ($mode=='gif'){
        header("Content-Type: image/gif; name=\"barcode_serial.gif\"");
        imagegif($im,'barcode_serial.gif');
    } else {
        header("Content-Type: image/png; name=\"barcode_serial.png\"");
        imagepng($im,'barcode_serial.png');
    }
...
function barcode_print($code, $encoding="ANY", $scale = 2 ,$mode = "png" ){
    ...
    else barcode_outimage($code, $bars['text'], $bars['bars'], $scale, $mode);
    return $bars;
}
