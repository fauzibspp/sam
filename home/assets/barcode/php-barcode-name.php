...
function barcode_outimage($folder = '', $text, $bars, $scale = 1, $mode = "png", $total_y = 0, $space = ''){
    ...
    if ($mode=='jpg' || $mode=='jpeg'){
       ...
       # header("Content-Type: image/jpeg; name=\"barcode_name.jpg\"");
       # imagejpeg($im,'barcode_name.jpg');

        $barcode_name = '../../documents/'.$folder.'/barcode_name.jpg';
        header("Content-Type: image/jpeg; name=\"$barcode_name\"");
        imagejpeg($im,$barcode_name);




    } else if ($mode=='gif'){
        header("Content-Type: image/gif; name=\"barcode_name.gif\"");
        imagegif($im,'barcode_name.gif');
    } else {
        header("Content-Type: image/png; name=\"barcode_name.png\"");
        imagepng($im,'barcode_name.png');
    }
...
function barcode_print($folder, $code, $encoding="ANY", $scale = 2 ,$mode = "png" ){
    ...
    else barcode_outimage($folder, $bars['text'], $bars['bars'], $scale, $mode);
    return $bars;
}
