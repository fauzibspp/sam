    ...
    <section class="wrapper site-min-height">
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                //$message = fnSQLCustom("select textbody from email_outbox where messageid='$id'","textbody");
                $sql = "select textbody from email_outbox where messageid='$id'";
                $rst = db_select($sql);
                $message = $rst[0]['textbody'];

                $sql_update = "UPDATE email_outbox SET is_read=1 WHERE messageid='$id'";
                db_query($sql_update);

                if($attach_file=='Y'){
                    $sql_attach = "SELECT d05_messageid as message_id, d05_filenamecurrent as filename,d05_filesize,
                     d05_filetype as filetype FROM
                            attachments_outbox WHERE d05_messageid ='".$id."'";// and d05_user_receipient='$v_owner'";
                    
                    $rows_attach = db_select($sql_attach);

                    
					$mail_response = 'id='.$id.'&subject='.$subject.'&from='.$from.'&to='.$to.'&date='.$date.'&attach_file=Y';
                    $mail_response = base64_url_encode($mail_response);
                }else{
				$mail_response = 'id='.$id.'&subject='.$subject.'&from='.$from.'&to='.$to.'&date='.$date.'&attach_file=N';
                    $mail_response = base64_url_encode($mail_response);
				}
                    ...
                    <div class="panel-body">
                        <div class="input-group-btn">
                                ...
                                <div class="btn-group hidden-phone">
                                    ...
                                    <ul class="dropdown-menu">
                                        <?php foreach($rows_attach as $row_attach):
                                            ...
                                            ?>
                                        <li>
                                            <a class="<?php echo $class ?>" href="../attachments/<?php echo $v_owner ?>/outbox/<?php echo $id ?>/<?php echo $row_attach['filename'] ?>" >
                            ...
                            <div class="btn-group-sm">
                                <a href="mail.outbox.forward.php?url=<?php echo base64_encode($mail_response) ?>" id="startForward" class="finish btn btn-primary btn-sm form-inline"><i class="icon-forward"></i> Forward</a>
                                <a href="mail.reply.php?url=<?php echo base64_encode($mail_response) ?>" id="startReply"  class="finish btn btn-primary btn-sm form-inline"><i class="icon-reply"></i> Reply</a>
                            </div>
