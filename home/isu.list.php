    ...
    <div id="dialog-form" aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" class="modal fade">
        <div class="modal-dialog table-responsive" >
                <div class="modal-content">
                    <form name="form_isu" id="form_isu" class="form-horizontal" role="form">
                    ...
                    <div class="modal-body">
                        ...
                        <input type="text" name="IsuName" id="IsuName" value="" class="form-control" placeholder="Nama Isu" />
                    </div>
                ...
                <div class="col-lg-12">
                    <section class="panel">
                        <header class="panel-heading">
                            <i class="icon-file"></i> Senarai Isu
                        </header>
                        <!--widget start-->
                        ...
                        <section class="panel">
							...
							<tbody>
								<?php 
									...
									$sql = 'SELECT
											id,
											nama_pertemuan as descriptions
											FROM
											class_pertemuan
											';
									$cnt =1;
								...
								?>
								<tr>
                                    <td><div id="checkboxes"><input type="checkbox" id="cbox" value="<?php echo $row['id'] ?>" name="addCB[]" class="cb" /></div></td>
									<td><?php echo $row['id'] ?></td>
									<td><?php echo $row['descriptions'] ?></td>
									<td>
                                        <a href="#dialog-form" onclick="jsEditData('<?php echo $row['id'] ?>','<?php echo $row['descriptions']  ?>')"  class="btn btn-default btn-xs pinda" data-toggle="modal"><i class="icon-pencil"></i></a>
...
$(document).ready(function(e) {
        ...
        $('.simpan').click(function(){
            var datastring = $("#form_isu").serialize();
            if($('#IsuName').val()==''){
                $('.alert').show();
        ...
        if(y = confirm('Anda pasti untuk teruskan?')){
            if(y){
                $.ajax({
                    type: 'post',
                    url: 'process.php',
                    data: datastring + '&token='+$('#token').val()+'&cmd=DelDataIsu',
                    success: function(html)
                    {
                        alert(html);
                        location.reload();
