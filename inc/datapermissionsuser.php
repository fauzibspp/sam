...
<div class="alert alert-block alert-danger fade in" style="display: none">
                        ...
                        foreach ($rst as $rows){
                            ...
                            $exist = db_check_data_exist("select role_user_id,role_permission_id from roleaccess_user where role_user_id='".$user_id."' and role_permission_id='".$rows['perm_id']."'");
                            if($exist===FALSE){
                            //if(fnCheckExistingRecord2Param($user_id,$rows['perm_id'],'roleaccess_user','role_user_id','role_permission_id')==0){
